...
namespace Blog.Core.Common.Helper
{
    public class JwtHelper
    {
        ...
        {
            string iss = "Audience：Issuer";
            string aud = "Audience：Audience";
            ...
            var claims = new List<Claim>
                ...
                new Claim(ClaimTypes.Name, tokenModel.Name),                          // 这里必须这样写 不然 HttpContext.User.Identity.Name 拿不到值
                new Claim(ClaimTypes.NameIdentifier, tokenModel.Name.ToString()),    // signalR 识别这个
                new Claim(JwtRegisteredClaimNames.Iat, $"{new DateTimeOffset(DateTime.Now).ToUnixTimeMilliseconds()}"),
                new Claim(JwtRegisteredClaimNames.Nbf,$"{new DateTimeOffset(DateTime.Now).ToUnixTimeSeconds()}") ,
                //这个就是过期时间，目前是过期1000秒，可自定义，注意JWT有自己的缓冲过期时间
                new Claim (JwtRegisteredClaimNames.Exp,$"{new DateTimeOffset(DateTime.Now.AddSeconds(Convert.ToInt16(AppSettingsHelper.app(new string[]{ "Jwt","ExpirationTime"})) )).ToUnixTimeSeconds()}"),
